...
describe("GameCard molecule", () => {
  ...
  it("submits a score and deletes via callbacks when admin", () => {
    const onSubmitScore = vi.fn<(gameId: number, score: string) => void>();
    const onDelete = vi.fn<(gameId: number) => void>();
  ...
  it("pauses a player via callback when admin", () => {
    const onPausePlayer = vi.fn<(gameId: number, memberId: number) => void>();
